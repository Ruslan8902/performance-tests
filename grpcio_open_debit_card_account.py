...
# Создаём gRPC-клиенты для UsersGatewayService и AccountsGatewayService
...
# Формируем запрос на создание дебитового счета с id созданного пользователя
open_debit_card_account_request = OpenCreditCardAccountRequest(
    user_id=create_user_response.user.id,
)

# Отправляем запрос и получаем ответ
open_debit_card_account_response: OpenCreditCardAccountResponse = accounts_gateway_service.OpenDebitCardAccount(open_debit_card_account_request)
